...
sub zip{
    ...
    $self->{job_metrics}->add("ingest_pack_seconds", $delta_time);
    my $bytes_w = -s $zip_path;
    $self->{job_metrics}->add("ingest_pack_bytes_read", $bytes_r);
    $self->{job_metrics}->add("ingest_pack_bytes_write", $bytes_w);
    $self->{job_metrics}->inc("ingest_pack_items");
    # success
