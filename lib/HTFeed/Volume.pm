...
sub new {
    ...
    $self->{nspkg} = HTFeed::Namespace->new(
	$self->{namespace},
	$self->{packagetype}
    );
...
sub get_identifier {
    my $self = shift;
    join(".", $self->get_namespace(), $self->get_objid());
}
...
sub get_all_directory_files {
	...
	next if $file =~ /^\.+$/ or $file eq '.DS_Store' or $file =~ /^thumbs\.db$/i;
	push(
	    @{ $self->{directory_files} },
	    $file
	);
...
sub get_sources {
    ...
    my $sth = $dbh->prepare(
	"SELECT content_provider_cluster, responsible_entity, n.collection, s.digitization_source, d.access_profile from feed_zephir_items n
	...
	if (!$access_profile) {
	    $self->set_error(
		"BadValue",
		field  => "digitization_agent",
		actual => $digitization_agents,
		detail => "Unexpected digitization agent(s) $digitization_agents for collection $collection"
	    );
	}
...
sub get_access_profile {
    ...
    my $dbh = HTFeed::DBTools::get_dbh();
    my $sth = $dbh->prepare(
	"SELECT collection, digitization_source FROM feed_zephir_items WHERE namespace = ? and id = ?"
    );
	...
	$self->set_error(
	    ...
	    detail => "Item not in feed_zephir_items; can't determine access profile"
	);
    }

    # try to map collection, digitization source to access profile
    $sth = $dbh->prepare(
	"SELECT access_profile FROM feed_collection_digitizers WHERE collection = ? and digitization_source = ?"
    );
	...
	$self->set_error(
	    ...
	    actual => "$collection / $digitization_source",
	    detail => "Unknown collection code or unexpected digitization source"
	);
    }
}
...
sub get_checksum_mets {
    ...
    my @nodes = $xpc->findnodes('//mets:file[@CHECKSUM][mets:FLocat/@xlink:href]');

    foreach my $node (@nodes) {
	my $checksum = $xpc->findvalue('./@CHECKSUM', $node);
	my $filename = $xpc->findvalue('./mets:FLocat/@xlink:href', $node);
	$checksums->{$filename} = $checksum;
    }
...
sub get_validation_overrides {
    ...
    if (
	$module eq 'HTFeed::ModuleValidator::TIFF_hul' and
...
sub should_check_validator {
    ...
    if (not defined $self->{note_from_mom} and defined $src_mets) {
	$xpc = $self->get_source_mets_xpc();
    }

    # get from database if there is no source METS file
    if (not defined $self->{note_from_mom} and not defined $src_mets) {
	...
	my @nodes = $xpc->findnodes(
	    '//premis:event[premis:eventType="manual inspection"]//htpremis:exceptionsAllowed/@category'
	);
	foreach my $exception_node (@nodes) {
	    push(
		@{$self->{note_from_mom}},
		$exception_node->getValue()
	    );
	}
    }
...
sub _parse_xpc {
    ...
    if ($@) {
	my $detail = $@;
	$detail    = $@->{message} if ref $@ eq 'HASH';
	$self->set_error("BadFile", file => $file, detail => $detail);
    } else {
	return $xpc;
    }
}
...
sub get_marc_xml {
	...
	"BadField",
	field  => "marcxml",
	file   => $self->get_source_mets_file(),
	detail => "Could not find MARCXML in source METS"
    );
...
sub get_page_count {
    ...
    unless (defined $image_group) {
	croak("Page count requested for object with no image filegroup");
    }
...
sub get_required_sequence_file_groups_by_page {
    ...
    return $self->get_file_groups_by_page(
	sub {
	    return ($_[0]->get_required() and $_[0]->get_sequence());
	}
    )
}
...
sub record_premis_event {
	...
	my $set_premis_sth = $dbh->prepare(
	    "REPLACE INTO feed_premis_events (namespace, id, eventtype_id, custom_xml) VALUES (?, ?, ?, ?)"
	);
	$set_premis_sth->execute(
	    $self->get_namespace(),
	    $self->get_objid(),
	    $eventcode,
	    $custom_event->toString()
	);
    } else {
	...
	my $set_premis_sth = $dbh->prepare(
	    "REPLACE INTO feed_premis_events (namespace, id, eventid, eventtype_id, outcome, date) VALUES (?, ?, ?, ?, ?, ?)"
	);
	$set_premis_sth->execute(
	    $self->get_namespace(),
	    $self->get_objid(),
...
sub clear_premis_events {
    ...
    my $objid = $self->get_objid();
    my $sth   = HTFeed::DBTools::get_dbh()->prepare(
	"DELETE FROM feed_premis_events WHERE namespace = ? and id = ?"
    );
...
sub remove_premis_event {
    ...
    my $objid     = $self->get_objid();
    my $sth       = HTFeed::DBTools::get_dbh()->prepare(
	"DELETE FROM feed_premis_events WHERE namespace = ? and id = ? and eventtype_id = ?"
    );
...
sub real_sip_location {
	...
	$self->set_error(
	    "MissingFile",
	    file   => $source,
	    detail => "can't find sip in ingest or failure location"
	);
	$source = undef;
    }
...
sub move_sip {
	...
	if (not -d dirname($target)) {
	    mkpath(dirname($target)) or $self->set_error(
		"OperationFailed",
		operation => "mkpath",
		file      => $target,
		detail    => $!
	...
	move($source, $target) or $self->set_error(
	    "OperationFailed",
	    operation => "move",
	    file      => $source,
	    detail    => $!
...
sub next_stage {
	...
	$self->set_error(
	    "UnexpectedError",
	    detail => "Action for stage $stage_name not defined"
	);
    }
