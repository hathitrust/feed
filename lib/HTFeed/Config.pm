...
BEGIN {
  $ENV{FEED_HOME} ||= realpath "$FindBin::Bin/..";
}

use lib "$ENV{FEED_HOME}/metslib";
...
sub init{
    my $feed_lib;
    my $feed_home = $ENV{FEED_HOME};

    # find bin, etc and var if they aren't predefined
    $feed_etc = "$feed_home/etc" if not defined $feed_etc or !-d $feed_etc;
    $feed_bin = "$feed_home/bin" if not defined $feed_bin or !-d $feed_bin;
    $feed_var = "$feed_home/var" if not defined $feed_var or !-d $feed_var;
    $feed_lib = "$feed_home/lib" if not defined $feed_lib or !-d $feed_lib;
