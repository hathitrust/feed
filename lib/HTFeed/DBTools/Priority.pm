...
sub set_item_priority{
    ...
    my $sth = get_dbh()->prepare(qq(UPDATE feed_queue SET priority = (priority & $BIN_MASK) + ? WHERE namespace = ? AND id = ?;));
...
sub group_priority{
    my ($ns,$pkg_type) = @_;
    my $get_group_priority = get_dbh()->prepare(q(SELECT MIN(priority) FROM feed_priority WHERE (namespace = ? AND pkg_type IS NULL) OR (namespace IS NULL AND pkg_type = ?) OR (namespace = ? AND pkg_type = ?);));
...
sub rebin{
    ...
    my $sth = get_dbh()->prepare(qq(UPDATE feed_queue SET priority = (priority & ~$BIN_MASK) + ? WHERE namespace = ? AND id = ? AND priority != $FIRST AND priority < $LAST;));
