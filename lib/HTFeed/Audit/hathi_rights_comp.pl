...
my $temp_dir="/htapps/ezbrooks.babel/tracker"; # <-- temp dir for testing. Change this for real run.
my $file = "$dFile" . ".gz";
...
my $first="select * from ht_files where ht_files.id not in (select rights_current.id from rights_current where ht_files.id=rights_current.id);";
...
my $second="select * from rights_current where rights_current.id not in (select ht_files.id from ht_files where rights_current.id=ht_files.id);";
