...
if ($one_line and $dot_packagetype) {
    pod2usage(
	-msg     => '-1 and -d flags incompatible',
	-exitval => 2
    );
}
if (($default_packagetype or $default_namespace) and ($one_line or $dot_packagetype)) {
    pod2usage(
	-msg => '-p and -n exclude -d and -1',
	-exitval => 2
    );
}
if ($reuse and !$reset_level) {
    pod2usage(
	-msg => 'can only use -u (reuse) when resetting'
    )
}
if ($one_line) {
...
if ($one_line) {
	...
	@volumes,
	HTFeed::Volume->new(
	    packagetype => $default_packagetype,
...
sub move_from_failed {
	...
	log_result(
	    $volume,
	    "punted item not reused - $failed_file doesn't exist"
	);
    }
	...
	log_result(
	    $volume,
	    "punted item not reused - won't overwrite $to_ingest_file"
	);
    }
